...
class SimulationStateSuite extends AnyFunSuite:
  ...
  test("currentTick returns the tick of the current scenario"):
    val state = SimulationState(states = LazyList.empty, current = atTick(2), running = true)

    state.currentTick shouldBe 2

  test("isFinished is true when there are no upcoming states left"):
    val state = SimulationState(states = LazyList.empty, current = atTick(1), running = true)

    state.isFinished shouldBe true

  test("isFinished is true once the current tick reaches maxIterations, even with states left"):
    val state =
      SimulationState(states = LazyList(atTick(4)), current = atTick(3), running = true)
  ...
  test(
    ...
    val state =
      SimulationState(states = LazyList(atTick(1)), current = atTick(0), running = true)
  ...
  test("next on an empty states LazyList leaves current unchanged and stops running"):
    val state = SimulationState(states = LazyList.empty, current = atTick(3), running = true)
